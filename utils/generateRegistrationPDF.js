...
export const generateRegistrationPDF = async (registration) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        ...
        .fontSize(16)
        .fillColor("#000000")
        .text("HÓA ĐƠN ĐĂNG KÝ PHÒNG", { align: "center" })
        .moveDown(1.5);
      ...
      doc
        .font("Bold")
        .fontSize(13)
        .text("Thông tin đăng ký phòng", { align: "left" })
        .moveDown(0.5);
      doc.moveTo(50, doc.y).lineTo(550, doc.y).stroke("#add8e6");

      doc.font("Regular").fontSize(11).moveDown(0.7);
      doc
        .text(`Mã đơn: ${registration.registrationCode}`, { continued: true })
      ...
      doc.text(registration.room.building.name, 50, tableTop + 25);
      doc.text(registration.room.room, 50 + columnSpacing, tableTop + 25);
      doc.text(registration.room.gender, 50 + columnSpacing * 2, tableTop + 25);
      doc.text(
        registration.room.price.toLocaleString() + " VND",
        50 + columnSpacing * 3,
        tableTop + 25
      );
      ...
      doc
        ...
        .fontSize(12)
        .fillColor("#000000")
        .text(`Tổng tiền: ${registration.room.price.toLocaleString()} VND`, {
          align: "right",
        });
      ...
      // Reset trạng thái
      doc.font("Italic").fontSize(10).fillColor("#4B5563");

      // Dùng width giới hạn để canh giữa thủ công nếu align không hoạt động
      const thankYouMessage =
        "Cảm ơn bạn đã đăng ký. Vui lòng chờ ban quản lý xử lý và theo dõi trạng thái duyệt trên hệ thống hoặc email.";
      const contactTitle = "Thông tin liên hệ";
      const contactInfo =
        "\nPhòng Quản lý Ký túc xá   |   SĐT: 0123 456 789   |   Email: ky­tucxa@example.com";

      // Tính độ rộng dòng để căn giữa thủ công
      ...
      // Canh giữa theo tọa độ X
      const centerX = (pageWidth - textWidth) / 2;
      const centerContactX = (pageWidth - contactWidth) / 2;
      const centerTitleX = (pageWidth - contactTitleWidth) / 2;

      // Hiển thị đoạn cảm ơn
      doc.text(thankYouMessage, centerX, doc.y).moveDown(1);

      // Hiển thị tiêu đề thông tin liên hệ
      doc
        .font("Bold")
        .fontSize(11)
        .fillColor("#000000")
